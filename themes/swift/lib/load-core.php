...
add_action('after_setup_theme', 'swift_functions', 3);
...
function swift_functions()
{
    ...
    require_once(trailingslashit(SWIFT_ADMIN) . 'font-stacks.php');
    ...
    require_once(trailingslashit(SWIFT_FUNCTIONS) . 'np-ajax-functions.php');
...
if (!function_exists('swift_setup')):
	...
	*/
        ...
        register_nav_menu('above-logo-tablets', __('Navigation menu above logo, for tablets', 'swift'));
        register_nav_menu('below-logo-tablets', __('Navigation menu below logo, for tablets', 'swift'));
        ...
        function swift_add_custom_styles()
        {
            ...
            global $swift_design_options, $swift_options;
            $upload_dir = wp_upload_dir();
            wp_enqueue_style('swift_custom_styles', $upload_dir['baseurl'] . '/swift-magic/custom-styles.css');

            /*
             * Moved to create-styles.php, 2 less http requests and more responsive for the morons.
             */
            if (isset($swift_design_options['enable_responsive']) && $swift_design_options['enable_responsive']):
                //wp_enqueue_style('swift_responsive_layout', get_template_directory_uri() . '/css/responsive.css');
                if (isset($swift_options['enable_np']) && $swift_options['enable_np'])
                    wp_enqueue_style('swift_np_responsive_layout', get_template_directory_uri() . '/css/np-responsive.css');
            endif;


        }

        add_action('wp_enqueue_scripts', 'swift_add_custom_styles', 1);
        ...
        function swift_add_scripts()
        {
            ...
            if (!isset($swift_design_options['disable_masonry']) || !$swift_design_options['disable_masonry'])
                wp_enqueue_script('masonry');
            $upload_dir = wp_upload_dir();
            wp_enqueue_script('swift_js', $upload_dir['baseurl'] . '/swift-magic/swift-js.js', array('jquery', 'jquery-ui-tabs'), '', TRUE);

            wp_localize_script('swift_js', 'swift', array('ajaxurl' => admin_url('admin-ajax.php'), 'fb_app_id' => $swift_options['sm_fb_app_id']));
